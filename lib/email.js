...
export async function notify({ subject, html }) {
  ...
  const from = process.env.NOTIFY_FROM || 'Would You Join Me for One Hour? <onboarding@resend.dev>';

  if (!key || !to) {
    console.warn('[email] RESEND_API_KEY or NOTIFY_EMAIL not set — skipping');
    return { skipped: true };
  }
