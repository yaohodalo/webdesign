...
export async function geocodeAddress(query) {
  ...
  let result = await nominatimSearch(trimmed);
  if (result) return result;

  // Nominatim often lacks specific house numbers — retry without the leading
  // street number so the street/city combination can still resolve.
  const withoutNumber = trimmed.replace(/^\d+\s+/, '');
  if (withoutNumber !== trimmed && withoutNumber.length >= 4) {
    result = await nominatimSearch(withoutNumber);
    if (result) return result;
  }
